...
impl<T: Hash + Eq> Heatmaps<T> {
    pub fn new(slices: usize) -> Heatmaps<T> {
        let t0 = Instant::now();
    ...
    pub fn increment(&mut self, key: T, start: Instant, value: u64) {
        self.increment_by(key, start, value, 1);
    }

    pub fn increment_by(&mut self, key: T, start: Instant, value: u64, count: u64) {
